...
export class IndexForward {
  ...
  private _index_state: IndexingState<Block>;
  ...
  public set index_state(index_state: IndexingState<Block>) {
  ...
  public get index_state(): IndexingState<Block> {
  ...
  > {
    ...
    if (
      ...
      this.deployment_latest_block >= chain_head_ptr.number
    ) {
    ...
    if (
      !this.deployment_latest_block ||
      chain_head_ptr.number - this.deployment_latest_block >=
      ...
      // let offset = latestBlock.number - this.deployment_latest_block - 1;
    }
  }
  ...
  private async next_blocks(): Promise<NextBlockReturns<Block>> {
    while (true) {
  ...
  private async process_block(block_with_triggers: BlockWithTriggers<Block>) {
  ...
  public async start() {
    ...
    switch (state) {
      ...
      case BlockStreamState.Idle: {
        while (true) {
          await wait(this.block_polling_interval);
        }
      }
      // }
      // }
